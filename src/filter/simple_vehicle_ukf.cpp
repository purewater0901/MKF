...
SimpleVehicleUKF::SimpleVehicleUKF() : augmented_size_(7),
                                       alpha_squared_(1.0),
                                       beta_(0.0),
                                       kappa_(3.0),
...
StateInfo SimpleVehicleUKF::predict(const StateInfo& state_info,
                                    ...
                                    const std::map<int, std::shared_ptr<BaseDistribution>>& measurement_noise_map)
{
    ...
    const auto dist_wu = system_noise_map.at(SYSTEM_NOISE::WU);
    const auto dist_wr = measurement_noise_map.at(OBSERVATION_NOISE::WR);
    const auto dist_wa = measurement_noise_map.at(OBSERVATION_NOISE::WA);

    Eigen::VectorXd augmented_mean = Eigen::VectorXd::Zero(augmented_size_);
    augmented_mean.head(3) = state_info.mean;
    augmented_mean(3) = dist_wv->calc_mean();
    augmented_mean(4) = dist_wu->calc_mean();
    augmented_mean(5) = dist_wr->calc_mean();
    augmented_mean(6) = dist_wa->calc_mean();
    ...
    augmented_cov(3, 3) = dist_wv->calc_variance();
    augmented_cov(4, 4) = dist_wu->calc_variance();
    augmented_cov(5, 5) = dist_wr->calc_variance();
    augmented_cov(6, 6) = dist_wa->calc_variance();
    ...
    for(size_t i=0; i<augmented_size_; ++i) {
        ...
        const Eigen::Vector3d processed_state = model_.propagate(sigma_points_.col(i).head(3), control_inputs, sigma_points_.col(i).segment(3, 2));
    ...
    }
    {
        ...
        const Eigen::Vector3d processed_state = model_.propagate(sigma_points_.col(2*augmented_size_).head(3), control_inputs, sigma_points_.col(2*augmented_size_).segment(3, 2));
...
StateInfo SimpleVehicleUKF::update(const StateInfo &state_info,
                                   ...
                                   const std::map<int, std::shared_ptr<BaseDistribution>>& measurement_noise_map)
{
    ...
    for(size_t i=0; i<2*augmented_size_+1; ++i) {
        const Eigen::Vector2d y = model_.observe(sigma_points_.col(i).head(3), sigma_points_.col(i).segment(5, 2), landmark);
